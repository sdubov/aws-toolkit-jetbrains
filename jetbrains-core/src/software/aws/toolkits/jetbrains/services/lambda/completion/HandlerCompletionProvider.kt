...
class HandlerCompletionProvider(private val project: Project) : TextCompletionProvider {
    ...
    // Suppress since gradle compiler mismatch return types for property delegate and fail build steps
    ...
    private val handlerCompletion: HandlerCompletion by lazy {
        ...
        if (runtimeGroup == null) {
            val message = "Unable to define Runtime Group for lambda handler completion context"
        ...
        if (completion == null) {
            val message = "Unable to get handler completion for lambda handler"
            logger.error(message)
            throw RuntimeException(message)
        }
